...
export function NatalChart({ chart }: Props) {
  ...
  return <div className="natal-chart-wrap">
    ...
    <details className="placement-table"><summary>{chartText('placementSummary', locale)}</summary><div className="table-scroll"><table aria-label={chartText('placements', locale)}><caption>{chartText('placements', locale)}</caption><thead><tr><th scope="col">{chartText('planet', locale)}</th><th scope="col">{chartText('zodiacPosition', locale)}</th><th scope="col">{chartText('house', locale)}</th><th scope="col">{chartText('status', locale)}</th></tr></thead><tbody>{planets.map((planet) => <tr key={planet.id}><th scope="row">{planet.glyph} {planetName(planet.id, locale)}</th><td>{formatZodiacDegree(planet.longitude, locale)}</td><td>{planet.house ? houseLabel(planet.house, locale) : chartText('unknownTime', locale)}</td><td>{motionName(planet.retrograde ? 'retrograde' : 'direct', locale)}</td></tr>)}</tbody></table></div></details>
  </div>;
}
