...
export const READING_COPY_KO: ReadingCopy = {
  ...
  'bigThree.pattern': ({ element, modality }) => `${elementPattern(String(element))}, ${modalityPattern(String(modality))}이 함께 작동합니다.`,
  ...
  'house.action': ({ house }) => `${houseName(Number(house), 'ko')}과 관련된 장면을 적어 성장 방향을 살펴보세요.`,
